	...
	def query(query,&proc)
		...
		loop do
			...
			if @skippages<id
			  ...
			  nextlinks=@page.links_with( :href => /scholar[?]start/ ).sort{|a,b| a.text.to_i <=> b.text.to_i }
        
        puts "%s (%s)" % [nextlinks.last.text, nextlinks.last.uri]
        n = nextlinks.last.text.to_i
        if 0<n and n<=(@skippages+1)
          nextlink=nextlinks.last
          id=n-1
				end
