...
echo "Merging files into filter_rel"
mkdir -p filter_rel
for i in `ls filter_rel1/` ; do
 # Note that -d removes duplicates
 $RUBY bibfilter.rb -d -i/./ "filter_rel1/$i" "filter_rel2/$i" > "filter_rel/$i" ;
done

echo "Compute intersection of files into filter_inter"
mkdir -p filter_inter
for i in `ls filter_rel1/` ; do
 # Note that -d removes duplicates
 $RUBY bibfilter.rb -d -i/./ --intersection "filter_rel1/$i" "filter_rel2/$i" > "filter_inter/$i" ;
done
...
Folders="filter_inter filter_rel1 filter_rel2 filter_rel filter_human downloads approaches"
